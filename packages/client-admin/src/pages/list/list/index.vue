...
const {
  ...
  queryKey: ['getListByPage', { page: pagination.value.current, pageSize: pagination.value.pageSize }],
  queryFn: () =>
    trpcClient.getListByPage.query({
...
const confirmBody = computed(() => {
  if (deleteIdx.value > -1 && isSuccess && data.value) {
    const { title } = data.value.list[deleteIdx.value];
    return `删除后，标题为 “${title}” 的项目列表将被清空，且无法恢复`;
  }
  return '';
});
...
const onConfirmDelete = async () => {
  if (isSuccess && data.value) {
    ...
    await trpcClient.deleteListById.mutate({ id: deleteId });
    // 判断删除的数据是否是当前页的第一个数据
...
const openDetailForm = async (item?: any) => {
  if (item) {
    dialogTitle.value = '修改项目列表';
    dialogFormState.value = 'updateNews';
    const id = item.row.id;
    const result = await trpcClient.getListById.query({ id });
    if (result && formData) {
      form.value?.reset();
      files.value = [];
      files.value?.push({
...
const onSubmit: FormProps['onSubmit'] = async ({ validateResult, firstError }) => {
  if (validateResult === true) {
    if (dialogFormState.value === 'addNews') {
      await trpcClient.addList.mutate({
        title: formData.title,
        desc: formData.desc,
        imgUrl: `http://localhost:3000${formData.imgUrl}`,
      });
    } else {
      await trpcClient.updateListById.mutate({
        id: formData.id,
        title: formData.title,
        desc: formData.desc,
    ...
    <t-card class="list-card-container" :bordered="false">
      <t-row justify="space-between">
        <div class="left-operation-container">
          <t-button @click="() => openDetailForm()">新增项目列表</t-button>
