...
const COLUMNS: PrimaryTableCol<TableRowData>[] = [
  ...
  { title: '密码', align: 'left', width: 100, colKey: 'password' },
  {
    title: '角色',
    align: 'left',
    width: 240,
...
const {
  ...
  queryKey: ['getUserByPage', { page: pagination.value.current, pageSize: pagination.value.pageSize }],
  queryFn: () =>
    trpcClient.getUserByPage.query({
...
watch(data, (newValue, _oldValue) => {
  if (newValue && newValue.userList.length !== 0) {
...
const confirmBody = computed(() => {
  if (deleteIdx.value > -1 && isSuccess && data.value) {
    const { name } = data.value.userList[deleteIdx.value];
...
const onConfirmDelete = async () => {
  if (isSuccess && data.value) {
    const deleteId = data.value.userList[deleteIdx.value].id;
    await trpcClient.deleteUserById.mutate({ id: deleteId });
    // 判断删除的数据是否是当前页的第一个数据
...
const openDetailForm = async (item?: any) => {
  if (item) {
    ...
    const result = await trpcClient.getUserById.query({ id });
    if (result && formData) {
      form.value?.reset();
      formData.id = result.id;
      formData.name = result.name;
      formData.password = result.password;
      formData.roleId = result.roleId;
...
const onSubmit: FormProps['onSubmit'] = async ({ validateResult, firstError }) => {
  if (validateResult === true) {
    if (dialogFormState.value === 'addNews') {
      await trpcClient.addUser.mutate({
        name: formData.name,
        password: formData.password,
        roleId: formData.roleId,
      });
    } else {
      await trpcClient.updateUserById.mutate({
        id: formData.id,
        name: formData.name,
        password: formData.password,
        roleId: formData.roleId,
  ...
  <div>
    ...
    <t-card class="list-card-container" :bordered="false">
      ...
      >
        ...
        <template #password="{ row }"> {{ row.password }}</template>
        <template #roleId="{ row }"> {{ row.roleId === 0 ? '管理员' : '普通用户' }} </template>
    ...
    >
      <t-space direction="vertical" style="width: 100%">
        <t-form labelAlign="left" ref="form" :data="formData" :rules="rules" @submit="onSubmit" preventSubmitDefault>
          <t-form-item label="用户名" name="name" initial-data="">
          ...
          <t-form-item label="角色" name="roleId" initial-data="">
            <t-radio-group v-model="formData!.roleId">
              <t-radio :value="0">管理员</t-radio>
              <t-radio :value="1">普通用户</t-radio>
            </t-radio-group>
          </t-form-item>
          <t-form-item>
            <t-space size="small">
